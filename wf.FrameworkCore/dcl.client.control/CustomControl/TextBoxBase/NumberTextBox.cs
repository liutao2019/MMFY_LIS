...
{
    public class NumberTextBox : TextBoxBase
    {
        private int textlenth = 0;//字符的长度
        ...
        public enum StyleSort
        {
            ...
            Integer = 1,//整数
            Decimal = 2,//小数
            PositiveInteger = 3,//正整数
        }

        private bool _keepZerosPrefix = false;
        [Browsable(true)]   //显示到属性栏
        [DefaultValue(false), Description("保留字符前的0")]
        public bool KeepZerosPrefix
        {
            get { return _keepZerosPrefix; }
            set
            {
                _keepZerosPrefix = value;
                base.Invalidate();
            }
        }
        ...
        {
            textlenth = ((TextBox)sender).Text.Length;
            ...
            else
            {
                ...
                for (int i = 0; i < this.Text.Length - 1; i++)//循环遍历文本框中的数值
                {
                    ...
                    if ((tem_char > '9' || tem_char < '0'))//如果字符不是数字
                    {
                        ...
                        {
                            //当前文本不能转换成数值型数据
                            //MessageBoxEx.Show(null, "无法将字符串转换成整数或小数！",
                            //"提示", MessageBoxIconEx.Information,
                            //MessageBoxButtonsEx.OK, AssemblyHelper.GetImage
                            // ("Microbe.BackGroundImg.BackgroundImage.jpg"));
                ...
                if (tem_BoolInt)//如果是数值型
                {
                                    ...
                                    case 3://保留指定的小数位数
                                        {
                                            ...
                                            if (Convert.ToInt32(this.DataStyle) == 2)
                                            {
                                                try
                                                {
                                                    string[] sArray = var_str.Split(new char[1] { '.' });
                                                    string IntNum = sArray[0].ToString();
                                                    string DemicalNum = sArray[1].ToString();
                                                    DemicalNum = DemicalNum.PadRight(ReservedDigit, '0');
                                                    string TotalNum = IntNum + "." + DemicalNum;
                                                    tem_value = Convert.ToDecimal(TotalNum);
                                                    //int dsg = var_str.IndexOf('.');
                                                    //string agd = var_str.Substring(0, var_str.IndexOf('.') + ReservedDigit);
                                                    //tem_value = Convert.ToDecimal(agd);
                                                }
                                                catch
                                                {
                                                }
                    ...
                    if(_keepZerosPrefix)
                        this.Text = tem_value.ToString().PadLeft(textlenth,'0');//显示保留后的数据
                    else
        ...
        public void Estimate_Key(KeyPressEventArgs e, string s, int n)
        {
            ...
            if (e.KeyChar == '-')//如果键值为'-'
                //如果“-”不在起始位输入，或以存在'-'
                if (this.SelectionStart != 0 && this.Text.Substring(0, 1) == "-"
