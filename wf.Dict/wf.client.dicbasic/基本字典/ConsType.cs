...
{
    public partial class ConsType : ConDicCommon, IBarActionExt
    {
        #region 保存数据表临时过滤信息
        /// <summary>
        /// 过滤编码信息
        ...
        public void DoRefresh()
        {
            ...
            if (isActionSuccess)
            {
                list = (ds.GetResult() as List<EntityDicPubProfession>).Where(w => w.ProType == 0).ToList();
        ...
        {
            if (gridView1.Columns["ProOrgName"].GroupIndex == 0)
            {
                bsType.Filter = "ProType = '专业组'";
                gridView1.Columns["ProOrgName"].GroupIndex = -1;
                gridView1.Columns["ProOrgName"].Visible = false;
            }
        }
        #endregion
