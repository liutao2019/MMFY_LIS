...
{
    public interface IDaoReaLossReport: IDaoBase
    {
        string GetReaSID_MaxPlusOne(DateTime date, string stepCode);
        bool ExsitSidOrHostOrder(string rea_sid, DateTime rea_in_date);
        bool InsertNewReaLossReport(EntityReaLossReport LossReport);
        List<EntityReaLossReport> QueryLossReportList(EntityReaQC reaQC);
        bool UpdateReaLossReportData(EntityReaLossReport reaLossReport);
        bool DeleteReaLossReportData(EntityReaLossReport reaLossReport);
        void UpdateLossReportStatus(string rayNo, string printId, DateTime date);
        bool ReturnReaLossReportData(EntityReaLossReport reaLossReport);
    }
}
