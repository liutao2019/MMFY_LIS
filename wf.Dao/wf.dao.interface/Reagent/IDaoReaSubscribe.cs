...
{
    public interface IDaoReaSubscribe : IDaoBase
    {
        string GetReaSID_MaxPlusOne(DateTime date, string stepCode);
        bool ExsitSidOrHostOrder(string rea_sid, DateTime rea_in_date);
        bool InsertNewReaSubscribe(EntityReaSubscribe Subscribe);
        List<EntityReaSubscribe> QuerySubscribeList(EntityReaQC reaQC);
        bool UpdateReaSubscribeData(EntityReaSubscribe reaSubscribe);
        bool DeleteReaSubscribeData(EntityReaSubscribe reaSubscribe);
        bool ReturnReaSubscribeData(EntityReaSubscribe reaSubscribe);
        void UpdateSubscribeStatus(string rayNo, string printId, DateTime date);
    }
}
