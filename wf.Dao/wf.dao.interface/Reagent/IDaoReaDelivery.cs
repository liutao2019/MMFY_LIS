...
{
    public interface IDaoReaDelivery: IDaoBase
    {
        string GetReaSID_MaxPlusOne(DateTime date, string stepCode);
        bool ExsitSidOrHostOrder(string rea_sid, DateTime rea_in_date);
        bool InsertNewReaDelivery(EntityReaDelivery Delivery);
        List<EntityReaDelivery> QueryDeliveryList(EntityReaQC reaQC);
        bool UpdateReaDeliveryData(EntityReaDelivery reaDelivery);
        bool DeleteReaDeliveryData(EntityReaDelivery reaDelivery);
        void UpdateDeliveryStatus(string rayNo, string printId, DateTime date);
        bool ReturnReaDeliveryData(EntityReaDelivery reaDelivery);
    }
}
