...
{
    public static class OutSendServiceBIZ
    {
        ...
        public static string UploadLisRepData(string reportResult)
        {
            ...
            // 开始组装命令，填充数据。
            //XmlNodeList nodes = document.SelectNodes("Report_Result/Report_Info");
            //if (nodes == null || nodes.Count <= 0)
            //    return "数据节点不存在！";


            //List<ReportInfoCommandAssemble> commands = new List<ReportInfoCommandAssemble>();
            //Logger.LogInfo("开始解析数据！");
            //foreach (XmlNode node in nodes)
            //{
            //    try
            //    {
            //        ReportInfoCommandAssemble reportCmd = GetInsertCommands(node);
            //        commands.Add(reportCmd);
            //        //InsertData(insertCmds);
            //    }
            //    catch (Exception ee)
            //    {
            //        //Logger.LogInfo(String.Format("{1} UploadLisRepData方法抛出异常:{0}", ee.Message, DateTime.Now));
            //        Logger.LogException(ee);
            //        errorMessage = errorMessage + ee.Message;
            //    }
            //}
            //Logger.LogInfo(String.Format("{1} UploadLisRepData插入数据:共{0}条!", commands.Count, DateTime.Now));
            ////?? 要开始执行命令插入数据,之前要做数据映射。
            //Logger.LogInfo("开始插入数据！");
            //int okCount = 0;
            //foreach (ReportInfoCommandAssemble cmd in commands)
            //{
            //    try
            //    {
            //        InsertReportInfoData(cmd);
            //        okCount = okCount + 1;
            //    }
            //    catch (Exception ee)
            //    {
            ...
            //    }
            //}
            //Logger.LogInfo(String.Format("{1} UploadLisRepData 成功插入数据:共{0}条; 失败：{2}!", okCount, DateTime.Now, commands.Count - okCount));
        ...
        public static string AffirmRequest(string hospSampleID)
        {
            ...
            try
            {
                ...
                if (result)
                {
                    ...
                    sampProcessDetail.ProcContent = OutSendServiceBIZ.GetUploadLisRepDataFromSysName() + "已接收";
        ...
        public static string RecordSampleStatus(string hospSampleID, string statusCode)
        {
            ...
            try
            {
                ...
                sampProcessDetail.ProcContent = OutSendServiceBIZ.GetUploadLisRepDataFromSysName() + "操作";
                new SampProcessDetailBIZ().SaveSampProcessDetailWithoutInterface(sampProcessDetail);
            }
            catch (Exception ee)
            {
    ...
    public class AssemblyLisCommand
    {
        ...
        /// 获取接口注册数据
        /// </summary>
        /// <param name="int_regno"></param>
        /// <returns></returns>
        private DataRow GetSysInterfaceRegeditInfo(string int_regno)
        {
            if (int_regno == null) int_regno = "";
            SqlDataAdapter adapter = new SqlDataAdapter("select * from sys_interface_regedit where int_regno= '" + int_regno + "' and int_regno is not null and int_regno<>''",
                OutSendServiceBIZ.GetSqlConnection_Lis());
        ...
        private string getPat_C_Name()
        {
            XmlNodeList nodes = reportInfoNode.SelectNodes("result_info");
            string result = "";
            foreach (XmlNode n in nodes)
            {
                XmlNode nn = n.SelectSingleNode("lis_combine_name");
                result = nn == null ? "" : nn.InnerText;
            }
            return result;
        }
        ...
        public void Execute()
        {
            ...
            try
            {
                ...
                {
                    //华银接口传完结果，医院检验科审核后，当华银再次传结果，会将原结果删除并插入新结果，导致原审核时写入的多参考值被删除，不利于医生查看结果判断结果
