...
namespace dcl.svr.interfaces
{
    ...
    internal class FTPManager
    {
        ...
        public FTPManager(string remoteHost, string remotePath, string remoteUser, string remotePass, int remotePort)
        {
        ...
        public string[] List(string strMask)
        {
            ...
            while (true)
            {
                ...
                //2016-07-21 林 这样判断有问题，要判断是否还有字节要接收
                //if (iBytes < buffer.Length)
                if (iBytes <= 0)
                {
                    break;
        ...
        public List<Byte[]> Get(string strFileNameMask)
        {
        ...
        {
            byte[] buff = GetByte(strRemoteFileName);
            string fileFullName = Path.Combine(strFolder, strLocalFileName);
            File.WriteAllBytes(fileFullName, buff);
            //if (!bConnected)
            //{
            //    Connect();
            //}
        ...
        /// 上传二进制流
        /// </summary>
        /// <param name="strFileName">远端文件名</param>
        public void Put(string strFileName, byte[] buff2)
        {
            if (!bConnected)
            {
                Connect();
