...
{
    public class ReaStorePositionBIZ : IDicCommon
    {
        ...
        public EntityResponse New(EntityRequest request)
        {
            ...
            if (request != null)
            {
                EntityDicReaStorePos sample = request.GetRequestValue<EntityDicReaStorePos>();
                IDaoDic<EntityDicReaStorePos> dao = DclDaoFactory.DaoHandler<IDaoDic<EntityDicReaStorePos>>();
                if (dao == null)
                {
        ...
        public EntityResponse Delete(EntityRequest request)
        {
            ...
            if (request != null)
            {
                EntityDicReaStorePos reaStorePosition = request.GetRequestValue<EntityDicReaStorePos>();
                reaStorePosition.del_flag = "1";
                IDaoDic<EntityDicReaStorePos> dao = DclDaoFactory.DaoHandler<IDaoDic<EntityDicReaStorePos>>();
                if (dao == null)
                {
                    response.Scusess = false;
                ...
                {
                    response.Scusess = dao.Update(reaStorePosition);
                }
            }
            else
            {
