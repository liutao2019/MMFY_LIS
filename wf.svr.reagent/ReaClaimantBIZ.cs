...
{
    public class ReaClaimantBIZ : IDicCommon
    {
        ...
        public EntityResponse New(EntityRequest request)
        {
            ...
            if (request != null)
            {
                EntityDicReaClaimant sample = request.GetRequestValue<EntityDicReaClaimant>();
                IDaoDic<EntityDicReaClaimant> dao = DclDaoFactory.DaoHandler<IDaoDic<EntityDicReaClaimant>>();
                if (dao == null)
                {
        ...
        public EntityResponse Delete(EntityRequest request)
        {
            ...
            if (request != null)
            {
                EntityDicReaClaimant reaClaimant = request.GetRequestValue<EntityDicReaClaimant>();
                reaClaimant.del_flag = "1";
                IDaoDic<EntityDicReaClaimant> dao = DclDaoFactory.DaoHandler<IDaoDic<EntityDicReaClaimant>>();
                if (dao == null)
                {
                    response.Scusess = false;
                ...
                {
                    response.Scusess = dao.Update(reaClaimant);
                }
            }
            else
            {
