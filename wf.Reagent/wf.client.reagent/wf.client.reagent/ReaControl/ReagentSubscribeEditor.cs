...
{
    public partial class ReagentSubscribeEditor : UserControl
    {
        public ReagentSubscribeEditor()
        {
            InitializeComponent();
        ...
        public ReagentSubscribeEditor(Istep istep)
        {
            InitializeComponent();
        ...
        private void txtReagentEdit_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            else if (e.KeyValue == 8)
            {
                if (this.listReaDetail != null && this.listReaDetail.Count > 0)
                {
                    ...
                    //对比最有项目，获取选中的项目
                    List<EntityReaSubscribeDetail> selectListDetail = listReaDetail.FindAll(i => i.ReagentName == strSelectRea_name);

                    if (selectListDetail.Count > 0 && selectListDetail.Count == 1)//如果只有一个项目组合
                    {
                        string rea_name = selectListDetail[0].ReagentName.ToString();
                        string rea_id = selectListDetail[0].Rsbd_reaid.ToString();
                    ...
                    else if (selectListDetail.Count > 0 && selectListDetail.Count > 1)//如果选择的项目有相同的其它组合
                    {
                        ...
                        for (int i1 = 0; i1 < selectListDetail.Count; i1++)
                        {
                            ...
                            if (intSelectStart == intTextIndex)
                            {
                                ...
                                string rea_id = selectListDetail[i1].Rsbd_reaid.ToString();
        ...
        private void ShowReagentSelectForm()
        {
            if (this.listReaDetail == null)
                listReaDetail = new List<EntityReaSubscribeDetail>();
            ...
            foreach (EntityReaSubscribeDetail item in listReaDetail)
            {
                EntityReaSetting Reagent = new EntityReaSetting();
                Reagent.Drea_id = item.Rsbd_reaid;
        ...
        private bool ExistReagent(string rea_id)
        {
            if (this.listReaDetail != null && this.listReaDetail.Count > 0)
            {
                if (this.listReaDetail.FindAll(i => i.Rsbd_reaid == rea_id).Count > 0)
                {
                    return true;
                }
            }
        ...
        private List<EntityReaSubscribeDetail> listPatMi;
        public List<EntityReaSubscribeDetail> listReaDetail
        {
            get
            {
        ...
        {
            List<EntityReaSubscribeDetail> reaListDetail = listReaDetail.FindAll(i => i.Rsbd_reaid == rea_id);
            foreach (EntityReaSubscribeDetail dr in reaListDetail)
            {
                dr.IsNew = 2;
        ...
        {
            if (listReaDetail != null && listReaDetail.FindAll(i => i.Rsbd_reaid == rea_id).Count == 0)
            {
                ...
                if (drReagent != null)
                {
                    ...
                    EntityReaSubscribeDetail drPatReaMi = new EntityReaSubscribeDetail();
                    drPatReaMi.Rsbd_reaid = rea_id;
