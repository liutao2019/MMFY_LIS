...
{
    public partial class ReagentEditor : UserControl, IReagentEditor
    {
        public ReagentEditor()
        {
            InitializeComponent();
        ...
        public ReagentEditor(Istep istep)
        {
            InitializeComponent();
        ...
        private void txtReagentEdit_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                if (this.listReaApplyDetail != null && this.listReaApplyDetail.Count > 0)
                {
                    ...
                    //对比最有项目，获取选中的项目
                    List<EntityReaApplyDetail> selectListDetail = listReaApplyDetail.FindAll(i => i.ReagentName == strSelectRea_name);

                    if (selectListDetail.Count > 0 && selectListDetail.Count == 1)//如果只有一个项目组合
                    {
                        string rea_name = selectListDetail[0].ReagentName.ToString();
                        string rea_id = selectListDetail[0].Rdet_reaid.ToString();
                    ...
                    else if (selectListDetail.Count > 0 && selectListDetail.Count > 1)//如果选择的项目有相同的其它组合
                    {
                        ...
                        for (int i1 = 0; i1 < selectListDetail.Count; i1++)
                        {
                            ...
                            if (intSelectStart == intTextIndex)
                            {
                                ...
                                string rea_id = selectListDetail[i1].Rdet_reaid.ToString();
        ...
        FrmReagentManagerV3 frmReagentSelectForm = null;
        ...
        {
            if (this.listReaApplyDetail == null)
                listReaApplyDetail = new List<EntityReaApplyDetail>();

            ReagentFilterParam3 param = new ReagentFilterParam3();
            param.group_id = this.GroupID;
            param.group_name = this.GroupName;

            if (frmReagentSelectForm == null)
            {
                frmReagentSelectForm = new FrmReagentManagerV3();
                frmReagentSelectForm.ReagentSelected += new FrmReagentManagerV3.ReagentSelectedEventHandler(frm_ReagentSelected);
            }

            frmReagentSelectForm.SetFilterParam(param);

            //给FrmReagentManagerV3界面的Reagent列表gridview赋值
            List<EntityReaSetting> reaList = new List<EntityReaSetting>();

            foreach (EntityReaApplyDetail item in listReaApplyDetail)
            {
                EntityReaSetting Reagent = new EntityReaSetting();
                Reagent.Drea_id = item.Rdet_reaid;
        ...
        private int GetYForPopupForm(FrmReagentManagerV3 frm)
        {
        ...
        private int GetXForPopupForm(FrmReagentManagerV3 frm)
        {
        ...
        void frm_ReagentSelected(object sender, string rea_id, EnumAction.ReagentEditorAction action)
        {
            if (this.listReaApplyDetail != null)
            {
                ...
                {
                    RemoveReagent(rea_id);
                    if (frmReagentSelectForm.Visible == false && listReaApplyDetail.Count > 0)
                    {
                        frmReagentSelectForm.Location = this.txtReagentEdit.Location;
                        //自定义弹出窗口的宽高
                        if (PopupFormWidth > 0)
        ...
        private bool ExistReagent(string rea_id)
        {
            if (this.listReaApplyDetail != null && this.listReaApplyDetail.Count > 0)
            {
                if (this.listReaApplyDetail.FindAll(i => i.Rdet_reaid == rea_id).Count > 0)
                {
                    return true;
                }
            }
        ...
        private List<EntityReaApplyDetail> listPatMi;
        public List<EntityReaApplyDetail> listReaApplyDetail
        {
            get
            {
        ...
        public void RefreshEditBoxText()
        {
            ...
            if (listReaApplyDetail != null && listReaApplyDetail.Count > 0)
            {
                //***************************************************************************//
                //根据组合进行排序，以达到在显示框中组合的名字能跟字典设置一样。
                //if (PatientsMi.Columns.Contains("com_seq"))
                //{
                int[] a = new int[listReaApplyDetail.Count];
                for (int i = 0; i < a.Length; i++)
                {
                    if (string.IsNullOrEmpty(listReaApplyDetail[i].sort_no))
                        a[i] = 99999;
                    else
                        a[i] = Convert.ToInt32(listReaApplyDetail[i].sort_no);
                }
                a = SortReagent(a);

                for (int i = 0; i < listReaApplyDetail.Count; i++)
                {
                    sb.Append(" + ");
                    sb.Append(listReaApplyDetail[a[i]].ReagentName);
                }
                //}
                //else
                //{
        ...
        {
            List<EntityReaApplyDetail> reaListDetail = listReaApplyDetail.FindAll(i => i.Rdet_reaid == rea_id);
            foreach (EntityReaApplyDetail dr in reaListDetail)
            {
                dr.IsNew = 2;
                listReaApplyDetail.Remove(dr);
            }
        ...
        {
            if (listReaApplyDetail != null && listReaApplyDetail.FindAll(i => i.Rdet_reaid == rea_id).Count == 0)
            {
                EntityReaSetting drReagent = CacheClient.GetCache<EntityReaSetting>().Find(i => i.Drea_id == rea_id);

                if (drReagent != null)
                {
                    int rea_seq = listReaApplyDetail.Count + 1;//13-5-17
                    EntityReaApplyDetail drPatReaMi = new EntityReaApplyDetail();
                    drPatReaMi.Rdet_reaid = rea_id;
                    drPatReaMi.ReagentName = drReagent.Drea_name;
                    drPatReaMi.sup_id = drReagent.Drea_supplier;
                    drPatReaMi.IsNew = 1;
                    drPatReaMi.sort_no = drReagent.sort_no.ToString();//组合排序号
                    listReaApplyDetail.Add(drPatReaMi);
        ...
        public enum ReagentEditorAction
        {
            Add,
            Remove,
        }
