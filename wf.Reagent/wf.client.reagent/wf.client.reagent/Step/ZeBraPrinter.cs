...
namespace wf.client.reagent
{
    ...
    public abstract class IPrintMachine
    {
        ...
        public abstract bool PrintReaBarcode(string machineName, string prtTemplate);
    ...
    public class ZeBraPrinter : IPrintMachine
    {
        ...
        {
            //wf.auxiliary.control.MessageDialog.Show("打印机名称：" + machineName, "提示");
        ...
        /// 试剂条码打印 
        /// </summary>    
        public override bool PrintReaBarcode(string machineName, string prtTemplate)
        {
            //wf.auxiliary.control.MessageDialog.Show("打印机名称：" + machineName, "提示");
            if (PrintInfo == null || this.PrintInfo.ID == null || PrintInfo.ID.Count == 0)
            {
            ...
            if (ConfigHelper.GetSysConfigValueWithoutLogin("Barcode_PrintType").Trim() == "单输出任务")
            {
                foreach (string bcID in listBCID)
                {
                    List<string> listSampSn = new List<string>();
                    listSampSn.Add(bcID);
                    DCLReportPrint.PrintReaBarCode(prtTemplate, listSampSn, machineName);
                }
            }
            else
            {
        ...
        public override bool Print(string machineName, string prtTemplate, string column, string allCombinesName)
        {
            ...
            {
                MessageDialog.Show("找不到条码机的打印模版", "提示");
                return false;
            }
        ...
        public override bool PrintByTJPace(string machineName, string prtTemplate)
        {
            ...
            //{
            //    wf.auxiliary.control.MessageDialog.Show("没有选择条码", "提示");
            //    return false;
            //}
            ...
            //if (prtTemplate == string.Empty)
            //{
            //    wf.auxiliary.control.MessageDialog.Show("找不到条码机的打印模版", "提示");
            //    return false;
            //}
